...
let io = IO(), INF = 9999999999
let (N, M) = (io.readInt(), io.readInt())
var graph = crtGraph()
var dist = [Int](repeating: INF, count: N+1)
var heap = Heap<Data>(Data(node: 1, cost: 0))

while !heap.isEmpty {
    let cur = heap.pop()!
    if cur.node == N { break }
    for (next, cost) in graph[cur.node] where cost+cur.cost < dist[next] {
        dist[next] = cost + cur.cost
...
func crtGraph() -> [[(Int, Int)]] {
    var result = Array(repeating: [(Int, Int)](), count: N+1)
    for _ in 0..<M {
        let (v, e, c) = (io.readInt(), io.readInt(), io.readInt())
        result[v].append((e, c))
        result[e].append((v, c))
    }
    return result
